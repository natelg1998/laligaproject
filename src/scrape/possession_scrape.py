...
if __name__ == "__main__":
    ...
    poss_df.drop(["Unnamed: 35_level_0_Match Report"], axis=1, inplace=True)  # Do not need these
    poss_df.to_csv(os.path.join(os.path.realpath("./data/fbref_data/"), r"possession.csv"))
    poss_df.to_sql('possession', con=conn, schema='laliga', if_exists='replace', index=False)
    end = time.time()
