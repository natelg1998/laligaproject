...
if __name__ == "__main__":
    ...
    passing_df.drop("Unnamed: 31_level_0_Match Report", axis=1, inplace=True)
    passing_df.columns = [
        "_date_",
        "_time_",
        "comp",
    ...
    passing_df.to_csv(
        os.path.join(os.path.realpath("./data/fbref_data/"), r"passing.csv")
    )
    passing_df.to_sql(
        "passing", con=conn, schema="laliga", if_exists="replace", index=False
    )
    conn.execute(
        """ALTER TABLE laliga.passing
